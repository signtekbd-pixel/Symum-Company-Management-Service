...
export function ProductCategoriesContent() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const url = editCategory ? `/api/product-categories/${editCategory.id}` : "/api/product-categories";
  ...
  async function handleDelete(id: string, name: string, productCount: number) {
    if (productCount > 0) {
      toast.error(`Cannot delete "${name}" — ${productCount} product(s) still use it.`);
      return;
    }
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500">Manage your product catalog categories</p>
        </div>
                  ...
                  <TableRow key={cat.id}>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-1">
                        ...
                        <Button variant="ghost" size="icon" className="text-red-600" onClick={() => handleDelete(cat.id, cat.name, cat._count.products)}>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md mx-4">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <Input value={formName} onChange={(e) => setFormName(e.target.value)} required placeholder="e.g. Business Cards" />
              </div>
              <div>
